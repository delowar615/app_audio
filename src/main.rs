    ...
    fn next(&mut self) -> Option<f32> {
        ...
        if self.fft_buffer.len() >= 1024 {
            ...
            for i in 0..16 {
                ...
                let mag = buffer[bin.min(511)].norm();
                self.shared.spectrum[i].store(((mag * 35.0).sqrt() * 10.0) as u32, Ordering::Relaxed);
            }
...
fn main() -> eframe::Result<()> {
    ...
    let state = Arc::new(Mutex::new(AppState {
        settings, playing: false, track_list,
    ...
    thread::spawn(move || {
        ...
        loop {
            let cmd = { let s = audio_state.lock().unwrap(); (s.playing, s.settings.cur_idx, s.settings.music_folder.clone(), s.settings.vol, s.should_skip, s.seek_request) };
                ...
                if let Some(name) = s.track_list.get(s.settings.cur_idx) {
                    if let Ok(file) = File::open(Path::new(&cmd.2).join(name)) {
                        if let Ok(source) = Decoder::new(BufReader::new(file)) {
                            ...
                            let dsp = DspEngine { inner: source.convert_samples::<f32>(), shared: Arc::clone(&audio_shared), fft_buffer: Vec::with_capacity(1024), planner: FftPlanner::new() };
    ...
    eframe::run_native("KOPEYSK ELITE RUST 23.9", eframe::NativeOptions::default(), Box::new(|_cc| Box::new(KopeyskApp { state, shared, cur_l: 0.0, cur_r: 0.0, bars: vec![0.0; 16] })))
}
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.vertical_centered(|ui| {
                ...
                // 1. ИНДИКАТОРЫ (320+20+320 = 660)
                ui.horizontal(|ui| {
                    ui.add_space((total_w - 660.0) / 2.0);
                    draw_meter_ui(ui, "L-CH", self.cur_l);
                    ui.add_space(20.0);
                    draw_meter_ui(ui, "R-CH", self.cur_r);
                });

                ui.add_space(40.0);

                // 2. БОБИНЫ И СПЕКТР (150 + 50 + 142 + 50 + 150 = 542)
                ui.horizontal(|ui| {
                    ui.spacing_mut().item_spacing.x = 0.0; // Отключаем авто-зазоры для точности
                    ui.add_space((total_w - 542.0) / 2.0);
                    
                    draw_reel_ui(ui, s.reel_angle); // 150
                    ui.add_space(50.0);             // 50
                    
                    // СПЕКТР (16 баров * 7 + 15 зазоров * 2 = 112 + 30 = 142)
                    for &h in &self.bars {
                        let rect = ui.allocate_at_least(egui::vec2(7.0, 80.0), egui::Sense::hover()).0;
                        ui.painter().rect_filled(rect.with_min_y(rect.bottom() - (h * 65.0).min(80.0)), 1.0, egui::Color32::from_rgb(255, 110, 0));
                        ui.add_space(2.0);
                    }
                    
                    ui.add_space(48.0);             // 48 (50 - 2 после последнего бара)
                    draw_reel_ui(ui, s.reel_angle); // 150
                });
                ...
                ui.label(egui::RichText::new(name).color(egui::Color32::from_rgb(255, 110, 0)).font(egui::FontId::proportional(20.0)));
                ...
                // 4. КНОПКИ (470px)
                ui.horizontal(|ui| {
                    ui.add_space((total_w - 470.0) / 2.0);
                    ...
                    ui.add_space(10.0);
                    if ui.add(egui::Button::new("NEXT").min_size(bs)).clicked() && !s.track_list.is_empty() {
                ...
                // 5. ГРОМКОСТЬ
                ui.horizontal(|ui| {
                    ui.add_space((total_w - 220.0) / 2.0);
                    ui.label("VOL");
                    if ui.add(egui::Slider::new(&mut s.settings.vol, 0.0..=1.0).show_value(false)).changed() { save_settings(&s.settings); }
                });
            });
        });
        ctx.request_repaint();
...
fn draw_meter_ui(ui: &mut egui::Ui, label: &str, val: f32) {
    ...
    for i in 0..11 {
        ...
        painter.line_segment([egui::pos2(center.x + a.cos()*radius, center.y + a.sin()*radius), 
            egui::pos2(center.x + a.cos()*(radius+15.0), center.y + a.sin()*(radius+15.0))], egui::Stroke::new(2.0, egui::Color32::BLACK));
    }
    let a_n = f32::to_radians(180.0 + 40.0 + ((val * 2.0).min(1.3)/1.3 * 100.0));
    painter.line_segment([center, egui::pos2(center.x + a_n.cos()*(radius+15.0), center.y + a_n.sin()*(radius+15.0))], egui::Stroke::new(4.5, egui::Color32::from_rgb(180, 0, 0)));
...
fn draw_reel_ui(ui: &mut egui::Ui, angle: f32) {
    ...
    for i in 0..3 {
        let a = angle + i as f32 * (2.0 * std::f32::consts::PI / 3.0);
        painter.line_segment([c, egui::pos2(c.x + a.cos()*60.0, c.y + a.sin()*60.0)], egui::Stroke::new(5.0, egui::Color32::from_rgb(255, 110, 0)));
    }
}
...
fn load_settings() -> Settings { File::open("settings.json").and_then(|f| Ok(serde_json::from_reader(f)?)).unwrap_or(Settings { vol: 0.5, cur_idx: 0, music_folder: "C:/Users/delowar/Music".into() }) }
