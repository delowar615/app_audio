...
impl<S: Source<Item = f32>> Iterator for LevelSpy<S> {
    ...
    fn next(&mut self) -> Option<f32> {
        ...
        self.sum_sq += s * s;
        self.count += 1;
        if self.count >= 512 { // Каждые 512 сэмплов обновляем уровень
...
fn main() -> eframe::Result<()> {
    ...
    let state = Arc::new(Mutex::new(AppState {
        ...
        should_skip: false, reel_angle: 0.0, seek_request: None,
    }));
    ...
    thread::spawn(move || {
        ...
        loop {
            ...
            let (play, idx, folder, vol, skip, seek) = cmd;
            sink.set_volume(vol);
            ...
            if let Ok(mut s) = audio_state.try_lock() {
                if play && !sink.empty() && !sink.is_paused() {
                    s.elapsed_time += Duration::from_millis(30);
                    s.reel_angle -= 0.04;
                } else if sink.is_paused() || sink.empty() {
                    audio_levels.l.store(0, Ordering::Relaxed);
                    audio_levels.r.store(0, Ordering::Relaxed);
                }
            }
    ...
    eframe::run_native("KOPEYSK ELITE RUST 21.0", options, Box::new(|_cc| Box::new(KopeyskApp { 
        state, levels, cur_l: 0.0, cur_r: 0.0 
    })))
}
...
impl eframe::App for KopeyskApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // БАЛЛИСТИКА 300мс (α ≈ 0.1 при 60FPS)
        let alpha = 0.1;
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ui.vertical_centered(|ui| {
                ...
                ui.add(egui::ProgressBar::new(elapsed / total).desired_width(700.0).text(format!("{:.1}s / {:.1}s", elapsed, total)));
                ui.add_space(40.0);
                ui.horizontal(|ui| {
                    ui.add_space(100.0);
                    if ui.button("PREV").clicked() && s.settings.cur_idx > 0 { s.settings.cur_idx -= 1; s.should_skip = true; }
                    if ui.button("REW").clicked() { s.seek_request = Some((elapsed - 10.0).max(0.0)); }
                    if ui.button(if s.playing { "PAUSE" } else { "PLAY" }).clicked() { s.playing = !s.playing; }
                    if ui.button("STOP").clicked() { s.playing = false; s.should_skip = true; s.elapsed_time = Duration::ZERO; }
                    if ui.button("FF").clicked() { s.seek_request = Some(elapsed + 10.0); }
                    if ui.button("NEXT").clicked() && !s.track_list.is_empty() {
                        s.settings.cur_idx = (s.settings.cur_idx + 1) % s.track_list.len(); s.should_skip = true;
                    }
                });
                ui.add_space(30.0);
                ui.horizontal(|ui| {
                    ui.add_space(280.0); ui.label("VOL");
...
// --- ОТРИСОВКА (БЕЗ ИЗМЕНЕНИЙ В ГЕОМЕТРИИ) ---
impl KopeyskApp {
    fn draw_meter_egui(&self, ui: &mut egui::Ui, label: &str, val: f32) {
        ...
        for i in 0..51 {
            ...
            pts.push(egui::pos2(center.x + a.cos() * radius, center.y + a.sin() * radius));
        }
        ...
        for i in 0..11 {
            ...
            painter.line_segment([
                ...
                egui::pos2(center.x + a.cos() * (radius + 15.0), center.y + a.sin() * (radius + 15.0)),
            ], egui::Stroke::new(2.0, egui::Color32::BLACK));
        }
        // СТРЕЛКА С УСИЛЕНИЕМ (x4.0) ДЛЯ ДИНАМИКИ
        let a = f32::to_radians(180.0 + 40.0 + ((val * 4.0).min(1.3)/1.3 * 100.0));
        painter.line_segment([center, egui::pos2(center.x + a.cos()*(radius+15.0), center.y + a.sin()*(radius+15.0))],
    ...
    fn draw_reel_egui(&self, ui: &mut egui::Ui, angle: f32) {
        ...
        for i in 0..3 {
            ...
            painter.line_segment([center, egui::pos2(center.x + a.cos()*(r-10.0), center.y + a.sin()*(r-10.0))],
                egui::Stroke::new(6.0, egui::Color32::from_rgb(255, 110, 0)));
        }
    }
}
    ...
    if let Ok(entries) = fs::read_dir(folder) {
        for entry in entries.flatten() {
            ...
            if let Some(ext) = p.extension() {
                if ["mp3", "wav", "flac"].contains(&ext.to_string_lossy().to_lowercase().as_str()) {
...
fn load_settings() -> Settings {
    ...
    if let Ok(file) = File::open(path) {
        if let Ok(s) = serde_json::from_reader(file) { return s; }
    }
    Settings { vol: 0.5, cur_idx: 0, music_folder: "C:/Users/delowar/Music".to_string() }
}
fn save_settings(s: &Settings) {
    if let Ok(file) = File::create("settings.json") { let _ = serde_json::to_writer_pretty(file, s); }
}
